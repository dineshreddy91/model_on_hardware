                    ...
                    uint32_t *sum, uint32_t *carry)
{
    ...
    // Generate the golden output for the toy fixed-point safety classifier.
    uint32_t required_separation_mm = 500u + (op_b >> 2);
    uint32_t request_stop = op_a <= required_separation_mm;

    // Compare CL vs Golden Model
    if (*sum == required_separation_mm && *carry == request_stop)
    {
        rc = 0;
        printf("PASS: distance=%u mm, closing_speed=%u mm/s, required=%u mm, action=%s\n",
               op_a, op_b, *sum, *carry ? "REQUEST_STOP" : "CLEAR");
    }
    else
    {
        rc = FPGA_ERR_SOFTWARE_PROBLEM;
        printf("FAIL: distance=%u mm, closing_speed=%u mm/s, required=%u mm, action=%u | "
               "Expected: required=%u mm, action=%u\n",
               op_a, op_b, *sum, *carry, required_separation_mm, request_stop);
    }
