...
int main(int argc, char **argv)
{
    int rc;
    int slot_id = 0;
    uint32_t value_a = 600;
    uint32_t value_b = 1000;
    uint32_t sum, carry;
    ...
    printf("===================================================\n");
    printf("Running toy safety-threshold inference\n");
    printf("===================================================\n");
    printf("slot_id = %d\n", slot_id);
    printf("distance_mm = %u\n", value_a);
    printf("closing_speed_mm_s = %u\n", value_b);
    printf("===================================================\n");
    ...
    // Run the classifier and compare the FPGA result with the golden model.
    rc = cl_add_validate(pci_bar_handle, value_a, value_b, &sum, &carry);
    fail_on(rc, out, "Addition validation failed");

    printf("Result: required_separation_mm = %u, action = %s\n",
           sum, carry ? "REQUEST_STOP" : "CLEAR");
    printf("TEST PASSED\n");
