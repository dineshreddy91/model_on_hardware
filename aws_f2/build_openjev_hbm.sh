...
source sdk_setup.sh
source hdk_setup.sh
export CL_DIR="${OPENJEV_CL_DIR:-/mnt/fpga-build/openjev-hbm-build/cl_dram_hbm_dma}"
...
cp "$openjev_script_dir/../full_model/rtl/openjev_int8_matvec.sv" "$CL_DIR/design/"
cp "$openjev_script_dir/../full_model/rtl/openjev_hbm_weight_reader.sv" "$CL_DIR/design/"
for name in build_all.tcl build_level_1_cl.tcl; do
  ln -sfn "$HDK_SHELL_DIR/build/scripts/$name" "$CL_DIR/build/scripts/$name"
done
cd "$CL_DIR/build/scripts"
"$HDK_SHELL_DIR/build/scripts/aws_build_dcp_from_cl.py" -c cl_dram_hbm_dma -t openjev_hbm_matvec_v2
