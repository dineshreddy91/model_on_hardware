...
module tb_openjev_hbm_element;
  ...
  reg [31:0] expected;
  openjev_hbm_element #(.WATCHDOG_CYCLES(80)) dut(.*);
  ...
  task access;
    ...
    begin
      ...
      request_write=write_flag;byte_offset=offset;element_bytes=width;write_data=data;request_valid=1;
      @(negedge clk);request_valid=0;
      deadline=0;
      while(!response_valid) begin
        @(negedge clk);deadline=deadline+1;
        if(fault||deadline>100) $fatal(1,"unexpected timeout/fault");
      end
  ...
  endtask
  task reset;
    begin rst_n=0;request_valid=0;repeat(3) @(negedge clk);rst_n=1;@(negedge clk);end
  endtask
  initial begin
    ...
    @(negedge clk);request_valid=0;repeat(90) @(negedge clk);
    if(!fault||fault_code!=4||response_valid) $fatal(1,"watchdog");
    freeze_bus=0;reset;rresp=2;
    request_valid=1;@(negedge clk);request_valid=0;repeat(20) @(negedge clk);
    if(!fault||fault_code!=1) $fatal(1,"read error");
    rresp=0;reset;
    if(fault||!request_ready) $fatal(1,"reset");
    $display("PASS HBM element: %0d accesses, 32 banks, strobes, AW/W stalls, bounds, protection, timeout, read error",tests);
    $finish;
  end
