...
module tb_openjev_attention;
  ...
  integer fd,outfd,n,case_id,i,cycle=0,received=0,deadline,case_count=0;
  string path,outpath;
  always #5 clk=~clk;
  assign rr=(cycle%3)!=0 && !pv;
  assign ready=(cycle%5)!=0;
  openjev_attention #(.MAX_KEYS(16),.MAX_DIM(256),.WATCHDOG_CYCLES(10000000)) dut(
