`timescale 1ns/1ps
module tb_openjev_table_ops;
  reg clk=0,rst_n=0,command_valid=0;
  always #5 clk=~clk;
  wire command_ready,memory_valid,memory_write,memory_response_ready,done,fault;
  reg [7:0] opcode=0;
  reg [31:0] token_count=0,channel_count=0,table_rows=0,image_count=0;
  reg [31:0] source0=0,source1=1,source2=2,source3=3,destination=4;
    ...
    if(!rst_n) begin cycle<=0;pending<=0;memory_response_valid<=0;memory_ready<=0;end
      ...
      if(pending) begin
        if(delay_count>0) delay_count<=delay_count-1;
          ...
          if(wr) begin
            if(tid!=4||index!=writes) $fatal(1,"write order");
  ...
  initial begin
    ...
    while(!$feof(fd)) begin
      n=$fscanf(fd,"%d %d %d %d %d %d\n",case_id,opcode,token_count,channel_count,table_rows,image_count);if(n!=6) $fatal(1,"header");
      ...
      expected_writes=(opcode==20 ? 1 : token_count)*channel_count;
    ...
    opcode=19;token_count=3;channel_count=2;image_count=2;
    memory[2][0]=1;memory[2][1]=1;writes=0;
    command_valid=1;@(negedge clk);command_valid=0;deadline=0;
    while(!fault) begin @(negedge clk);deadline=deadline+1;if(done||deadline>1000) $fatal(1,"duplicate image slot accepted");end
    if(writes!=0) $fatal(1,"wrote before slot preflight");
    repeat(3) @(negedge clk);if(!fault||command_ready) $fatal(1,"fault not sticky");
    rst_n=0;repeat(3) @(negedge clk);rst_n=1;@(negedge clk);
    opcode=16;table_rows=4;memory[0][0]=32'hffffffff;
    command_valid=1;@(negedge clk);command_valid=0;deadline=0;
    while(!fault) begin @(negedge clk);deadline=deadline+1;if(done||deadline>1000) $fatal(1,"negative index accepted");end
    if(writes!=0) $fatal(1,"invalid index committed");
    rst_n=0;repeat(3) @(negedge clk);rst_n=1;@(negedge clk);
    opcode=20;memory_response_error=1;
    command_valid=1;@(negedge clk);command_valid=0;deadline=0;
    ...
    $fclose(outfd);$display("PASS table operators: stalls, committed writes, duplicate slots, bounds and bus faults");$finish;
  end
endmodule
