...
module openjev_attention #(
  ...
  typedef enum logic [5:0] {IDLE,READ_ISSUE,READ_WAIT,ALU_ISSUE,ALU_WAIT,
    ...
    VALUE_MUL,VALUE_ADD,VALUE_SAVE,VALUE_NEXT,EMIT,FAULTED} state_t;
  state_t state,read_return,alu_return;
  logic [63:0] cycles;
  logic [31:0] qmem[0:MAX_DIM-1],scores[0:MAX_KEYS-1];
  ...
  always_ff @(posedge clk) begin
    if(!rst_n) begin
      ...
      case(state)
        ...
        Q_FETCH: fetch(0,qi*dim+di,Q_SAVE);
        Q_SAVE: begin
          qmem[di]<=loaded;
          if(di==dim-1) begin di<=0; kj<=0; any_key<=0; maximum<=32'hff7fffff; state<=KEY_START; end
        ...
        KEY_DECIDE: begin
          enabled[kj]<=key_enabled;
          if(key_enabled) begin sum<=0; di<=0; any_key<=1; state<=K_FETCH; end
          else begin scores[kj]<=0; state<=KEY_NEXT; end
        end
        K_FETCH: fetch(1,kj*dim+di,DOT_MUL);
        ...
        KEY_NEXT: begin
          ...
          end else begin kj<=kj+1; state<=KEY_START; end
        end
        EXP_START: begin
          if(enabled[kj]) state<=EXP_ARG;
          else if(kj==nk-1) state<=INVERSE;
          else kj<=kj+1;
        end
        EXP_ARG: calculate(0,scores[kj],{~maximum[31],maximum[30:0]},EXP_EVAL);
        EXP_EVAL: calculate(2,result,0,EXP_SAVE);
        EXP_SAVE: begin scores[kj]<=result; calculate(0,sum,result,SUM_SAVE); end
        SUM_SAVE: begin
          sum<=result;
        ...
        PROB_START: begin
          if(enabled[kj]) calculate(1,scores[kj],inverse,PROB_SAVE);
        ...
        VALUE_START: begin kj<=0; sum<=0; state<=VALUE_FETCH; end
        VALUE_FETCH: begin
          if(enabled[kj]) fetch(2,kj*dim+vd,VALUE_MUL);
          else state<=VALUE_NEXT;
        end
        VALUE_MUL: calculate(1,scores[kj],loaded,VALUE_ADD);
        VALUE_ADD: calculate(0,sum,result,VALUE_SAVE);
        VALUE_SAVE: begin sum<=result; state<=VALUE_NEXT; end
        VALUE_NEXT: begin
          if(kj==nk-1) state<=EMIT;
          else begin kj<=kj+1; state<=VALUE_FETCH; end
        end
        EMIT: if(output_ready) begin
          if(vd!=dim-1) begin vd<=vd+1; state<=VALUE_START; end
          else if(qi!=nq-1) begin qi<=qi+1; di<=0; state<=Q_FETCH; end
